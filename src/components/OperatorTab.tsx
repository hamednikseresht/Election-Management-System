...
export function OperatorTab({ data, setData }: OperatorTabProps) {
  ...
  return (
    <div className="h-full overflow-y-auto p-3 sm:p-6 font-sans text-slate-900">
        ...
        <div className="rounded-3xl p-5 shadow-sm border border-slate-200 bg-white text-slate-800 flex flex-col gap-3">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2.5">
              <span className="flex items-center justify-center w-7 h-7 rounded-lg bg-sky-700 text-white font-black text-xs">
                ۱
              </span>
              <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-1">
            ...
            <button
              ...
              className={`flex items-start gap-3.5 p-3.5 sm:p-4 rounded-2xl border text-right transition-all cursor-pointer ${
                isSingleScope
                  ? 'border-sky-700 bg-sky-50/70 ring-2 ring-sky-600/20 shadow-xs'
                  : 'border-slate-200 bg-slate-50/60 hover:bg-slate-100 hover:border-slate-300'
              }`}
            >
              <div className={`p-2.5 rounded-xl shrink-0 ${
                isSingleScope ? 'bg-sky-700 text-white' : 'bg-slate-200 text-slate-600'
              }`}>
                <Vote size={22} />
              </div>
              <div className="flex-1">
                <div className="text-sm font-black text-slate-900 flex items-center justify-between">
                  ...
                  {isSingleScope && <Check size={18} className="text-sky-700" />}
                </div>
            ...
            <button
              ...
              className={`flex items-start gap-3.5 p-3.5 sm:p-4 rounded-2xl border text-right transition-all cursor-pointer ${
                !isSingleScope
                  ? 'border-slate-700 bg-slate-50/70 ring-2 ring-slate-600/20 shadow-xs'
                  : 'border-slate-200 bg-slate-50/60 hover:bg-slate-100 hover:border-slate-300'
              }`}
            >
              <div className={`p-2.5 rounded-xl shrink-0 ${
                !isSingleScope ? 'bg-slate-700 text-white' : 'bg-slate-200 text-slate-600'
              }`}>
                <Columns size={22} />
              </div>
              <div className="flex-1">
                <div className="text-sm font-black text-slate-900 flex items-center justify-between">
                  ...
                  {!isSingleScope && <Check size={18} className="text-slate-700" />}
                </div>
                <p className="text-xs text-slate-500 mt-1 leading-relaxed">
            ...
            <div className="rounded-3xl p-4 shadow-sm border border-slate-200 bg-white text-slate-800 flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                <div className="flex items-center gap-1.5 text-xs font-bold text-slate-700">
                  <MonitorPlay size={16} className="text-sky-700" />
                ...
                <div className="flex p-1 rounded-xl border border-slate-200 bg-slate-100 text-xs">
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-bold transition-all cursor-pointer ${
                      data.displayMode === 'dual'
                        ? 'bg-gradient-to-r from-sky-700 to-slate-700 text-white shadow-xs'
                        : 'text-slate-600 hover:text-slate-900'
                    }`}
                  ...
                  >
                    <Vote size={14} className={data.displayMode === 'single-1' ? 'text-white' : 'text-sky-700'} />
                  ...
                  >
                    <ShieldCheck size={14} className={data.displayMode === 'single-2' ? 'text-white' : 'text-slate-700'} />
